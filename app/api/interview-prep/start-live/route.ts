...
import { addInterviewMessage, createLiveInterviewSession } from "@/lib/interview-prep/data";
import { generateFirstLiveQuestion } from "@/lib/interview-prep/ai";
...
export async function POST(request: Request) {
  ...
  const session = await createLiveInterviewSession(supabase, user.id, prep.id, prep.job_application_id);
  if (!session) return NextResponse.json({ error: "Could not create live session" }, { status: 500 });

  const first = await generateFirstLiveQuestion({
  ...
  const firstQuestion = first?.first_question?.trim();
  if (!firstQuestion) {
