...
const flagFromCountry = (country?: string | null, mode?: string | null) => (mode === "remote" ? <Plane size={16} className="inline" /> : <span>{country ? country.slice(0, 2).toUpperCase().replace(/./g, (c) => String.fromCodePoint(127397 + c.charCodeAt(0))) : "-"}</span>);
...
export function JobDetailsClient({ data }: { data: any }) {
  ...
  const summaryItems = useMemo(() => [
    ...
    { label: "Country", value: row.country || "Remote" },
