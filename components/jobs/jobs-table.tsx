...
export function JobsTable({ initialData, userId }: { initialData: any[]; userId: string }) {
  ...
  const filteredData = useMemo(() => {
    ...
    return rows.sort((a, b) => {
      ...
      if (sortBy === "applied_at") {
        ...
        return sortDir === "desc" ? rightDate - leftDate : leftDate - rightDate;
      }
  ...
  return <div className="space-y-4">
    ...
    <div className="flex flex-wrap gap-2"><Input placeholder="Job title" value={filter.title} onChange={(e) => setFilter({ ...filter, title: e.target.value })} className="w-full sm:w-40" /><Input placeholder="Company" value={filter.company} onChange={(e) => setFilter({ ...filter, company: e.target.value })} className="w-full sm:w-40" /><Select value={filter.status} onChange={(e) => setFilter({ ...filter, status: e.target.value })} className="w-full sm:w-40"><option value="">All status</option>{statusOptions.map((s) => <option key={s}>{s}</option>)}</Select><Select value={sortBy} onChange={(e) => setSortBy(e.target.value as any)} className="w-full sm:w-44"><option value="applied_at">Applied date</option><option value="job_title">Job title</option><option value="company_name">Company</option></Select><Select value={sortDir} onChange={(e) => setSortDir(e.target.value as any)} className="w-full sm:w-40"><option value="desc">Descending (Z-A / Newest)</option><option value="asc">Ascending (A-Z / Oldest)</option></Select><Button variant="danger" disabled={!selectedIds.length} onClick={deleteSelected}>Delete Selected ({selectedIds.length})</Button></div>
